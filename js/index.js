V5.registerPage("index", function () {
    ...
    var initialize = function (id) {
        ...
        var page = this,
            view = V5.View(page.node),
            contentBox = view.$('textarea'),
            contentBuffer = view.$('#content-buffer'),
            proxy = new EventProxy(),
            _notes = JSON.parse(localStorage.notes),
            noteid = location.pathname.substring(1).replace(/\.html/, '');
        if (noteid) {
            $.get('/getnote?id=' + noteid, function (msg) {
                contentBox.val(msg);
                contentBuffer.html(contentBox.val().replace(/\n/g, '<br />'));
                contentBox.css('height', contentBuffer.css('height'));
                iscroll.refresh();
            });
        }
        ...
        view.bind("input", function (evnet) {
            contentBuffer.html(contentBox.val().replace(/\n/g, '<br />'));
            contentBox.css('height', contentBuffer.css('height'));
            iscroll.refresh();
        });
