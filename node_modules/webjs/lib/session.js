...
session.start = function (req, res) {
	...
	if (typeof get[session.SESSION_TAG] == 'string' && get[session.SESSION_TAG] != '')
		...
		if (typeof req.cookie[session.SESSION_TAG] == 'string')
			var session_id = req.cookie[session.SESSION_TAG];
		// 否则，创建一个新的Session id
		else
			var session_id = md5(new Date().getTime() + '' + Math.random());
		// 如果是通过cookie设置的session id，则需要更新cookie的存活期限
		res.cookie(session.SESSION_TAG, session_id, {maxAge: session.MAX_AGE});
	}
...
setInterval(function () {
	console.log('[debug]  session recover...');
